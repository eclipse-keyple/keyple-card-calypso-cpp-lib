...
class FreeTransactionManagerAdapterTest
: public ::testing::Test,
...
{
    EXPECT_THROW(
        cardTransactionManager->prepareReadBinary(
            static_cast<uint8_t>(-1), 1, 1),
        IllegalArgumentException);
}
    ...
    prepareSearchRecords_whenSfiIsNegative_shouldThrowIAE)
{
    ...
    data->setSfi(static_cast<uint8_t>(-1)).setSearchData({0});
    ...
    prepareUpdateBinary_whenSfiIsNegative_shouldThrowIAE)
{
    EXPECT_THROW(
        cardTransactionManager->prepareUpdateBinary(
            static_cast<uint8_t>(-1), 1, std::vector<std::uint8_t>(1)),
        IllegalArgumentException);
}
