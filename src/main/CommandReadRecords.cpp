             ...
             != WriteAccessLevel::UNKOWN)
{
    ...
    const std::uint8_t p1 = static_cast<std::uint8_t>(firstRecordNumber);
    std::uint8_t p2
        = static_cast<std::uint8_t>((sfi == 0x00) ? 0x05 : ((sfi * 8) + 5));
    ...
    const std::uint8_t le = static_cast<std::uint8_t>(
...
CommandReadRecords::parseResponse(std::shared_ptr<ApduResponseApi> apduResponse)
{
    ...
    if (mReadMode == CommandReadRecords::ReadMode::ONE_RECORD) {
        getTransactionContext()->getCard()->setContent(
            ...
            static_cast<std::uint8_t>(mFirstRecordNumber),
            dataOut);
    } else {
...
CommandReadRecords::buildAnticipatedResponse()
{
    std::shared_ptr<ElementaryFile> ef
        = getTransactionContext()->getCard()->getFileBySfi(
            static_cast<std::uint8_t>(mSfi));
    if (ef == nullptr) {
        return {};
    }
    ...
    const std::shared_ptr<ElementaryFile>& ef)
{
    ...
    if (content.size() > 0
        ...
        const int length = expectedResponseLength != 0
    ...
    const std::shared_ptr<ElementaryFile>& ef)
{
    ...
    for (int i = mFirstRecordNumber; i <= lastRecordNumber; i++) {
        const std::vector<std::uint8_t> content
            = ef->getData()->getContent(static_cast<std::uint8_t>(i));
        if (static_cast<int>(content.size()) >= mRecordSize) {
            /* Record number */
            apdu[index++] = static_cast<std::uint8_t>(i);
            /* Record size */
            apdu[index++] = static_cast<std::uint8_t>(mRecordSize);
            /* Record content */
