...
const std::map<int, const std::shared_ptr<Command::StatusProperties>>
    CommandUpdateRecord::STATUS_TABLE = [] {
        ...
        m.insert(
            {{0x6400,
             ...
             {0x6981,
              std::make_shared<Command::StatusProperties>(
                  "Command forbidden on cyclic files when the record exists "
                  "and "
                  "is not record 01h and on binary files",
...
void
CommandUpdateRecord::finalizeRequest()
{
    encryptRequestAndUpdateTerminalSessionMacIfNeeded();
}

bool
CommandUpdateRecord::isCryptoServiceRequiredToFinalizeRequest() const
{
    return getCommandContext()->isEncryptionActive();
}

bool
CommandUpdateRecord::synchronizeCryptoServiceBeforeCardProcessing()
{
...
void
CommandUpdateRecord::parseResponse(
    std::shared_ptr<ApduResponseApi> apduResponse)
{
    decryptResponseAndUpdateTerminalSessionMacIfNeeded(apduResponse);
    Command::setApduResponseAndCheckStatus(apduResponse);
    getTransactionContext()->getCard()->setContent(mSfi, mRecordNumber, mData);
    updateTerminalSessionIfNeeded();
}

const std::map<int, const std::shared_ptr<Command::StatusProperties>>&
CommandUpdateRecord::getStatusTable() const
{
    return STATUS_TABLE;
}
