...
/**
 * Defines the two existing ISO7816 class bytes for a Calypso card command.:
 * LEGACY for REV1 / BPRIME type card, ISO for REV2/3 / B type
 *
 * @since 2.0.0
 */
class KEYPLECARDCALYPSO_API CalypsoCardClass {
public:
    /** Dummy init value */
    ...
    operator<<(std::ostream& os, const CalypsoCardClass& ccc);
...
private:
     ...
     */
    explicit CalypsoCardClass(std::uint8_t cla);
};
